...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = (referenceImageUrls?.length > 0) 
      ? await NSFWService.generateFromImage(prompt, referenceImageUrls, parameters)
      : await NSFWService.generateImage(prompt, parameters);
    ...
    const { data: mediaData, error: dbError } = await supabase
      ...
      .insert([{
        ...
        url: permanentUrl,
        type: 'photo',
        content_type: contentType || 'nsfw', // Important : SFW ou NSFW
        prompt: prompt,
